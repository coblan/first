 var app = angular.module('share',['ngSanitize']);
...
app.directive('editForm',function ($timeout) {
	...
	return {
		...
		//	$rootScope.title="2016MOKI年会";
		//});
		
		//app.factory('wei',function () {
		//	return {
		//		cst_unOpenids: function (items) {
		//			var uncomplete_user = $.grep(items,function (item) {
		//					if(!item.nickname){
		//						return true;
		//					}else{
		//						return false;
		...
		//				});
		//			var un_openids=[];
		//			for (x in uncomplete_user){
		//				un_openids.push(uncomplete_user[x].openid);
		//			}
		//			return un_openids;
		//		},
		//		update_unUserinfo : function (userinfos,old_users) {
		//				    for (x in userinfos){
		//					    for (y in old_users){
		//						    if(old_users[y].openid == userinfos[x].openid){
		//							    old_users[y].nickname = userinfos[x].nickname;
		//							    old_users[y].head = userinfos[x].headimgurl;
		//						    }
		//					    }
		//				    }
		//		},
